...
// Use a more robust replacement that doesn't depend on corrupt symbols
const productsBlockStart = "      // Model mapping untuk kegunaan sistem";
const productsBlockEnd = "      }\n    };";

const startIndex = content.indexOf(productsBlockStart);
const endIndex = content.indexOf(productsBlockEnd, startIndex);

if (startIndex !== -1 && endIndex !== -1) {
    const newProducts = `      // Model mapping untuk kegunaan sistem
      products: {
    ...
    console.log('Successfully updated KRONOS_CONFIG products.');
} else {
    console.log('Error: Could not find products block in KRONOS_CONFIG.');
    process.exit(1);
}
